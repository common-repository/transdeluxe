...
/*  Copyright 2010 TransDeluxe  (email : help@transdeluxe.com)
...
function gltr_get_flags_bar() {
  ...
  $num_cols = BAR_COLUMNS;
	if (!isset($gltr_engine) || $gltr_engine == null ){
		...
		return "<b>TransDeluxe not configured yet. Please go to the Options Page</b>";
	}
  ...
  if (HTML_BAR_TAG == 'MAP' && !file_exists($gltr_merged_image)){
    if (!is_writeable(dirname(__file__))){
      return "<b>Permission error: Please make your 'plugins/transdeluxe' directory writable by Wordpress</b>";
    } else {
  ...
  $n2hlink = "<a style=\"font-size:9px;\" href=\"http://www.transdeluxe.com\">By TransDeluxe</a>";
  if (HTML_BAR_TAG == 'MAP'){
    $buf .="</map>";
...
function gltr_mkdir($dirtomake){
  if (!is_dir($dirtomake)) { 
    if (!@mkdir($dirtomake, 0777)){
     	die("<b>TransDeluxe has detected a problem with your filesystem permissions:<br />The cache dir <em>$dirtomake</em> cannot be created. <br />Please make readable and writeable the following directory: <br /><em>".WP_CONTENT_DIR."</em>.</b>");
    }
    if(!file_exists($dirtomake) || !is_readable($dirtomake) || !is_writeable($dirtomake)){
    	die("<b>TransDeluxe has detected a problem with your filesystem permissions:<br />The cache dir <em>$dirtomake</em> cannot be read or modified. <br />Please chmod it in order to make it readable and writeable.</b>");
    }
  }
...
function gltr_get_page_content($lang, $url) {
  ...
  if(file_exists($filename) && (!is_readable($filename) || !is_writeable($filename))){
  	return "<b>TransDeluxe has detected a problem with your filesystem permissions:<br />The cached file <em>$filename</em> cannot be read or modified. <br />Please chmod it in order to make it readable and writeable.</b>";
  }
  if(file_exists($stale_filename) && (!is_readable($stale_filename) || !is_writeable($stale_filename))){
  	return "<b>TransDeluxe has detected a problem with your filesystem permissions:<br />The cached file <em>$stale_filename</em> cannot be read or modified. <br />Please chmod it in order to make it readable and writeable.</b>";
  }
  ...
  if (isset($query->query_vars['gtlang'])) {
    ...
    if (!is_dir($gltr_cache_dir)){
      if (!is_readable(WP_CONTENT_DIR) || !is_writable(WP_CONTENT_DIR) ){
        ...
        die ("Unable to complete TransDeluxe initialization. Plese make writable and readable the following directory:
...
function gltr_add_options_page() {
 ...
 add_options_page('TransDeluxe Options','TransDeluxe',8,$file);
}
...
function gltr_debug($msg) {
  if (DEBUG) {
    ...
    $fh = fopen($myFile, 'a') or die("Can't open debug file. Please manually create the 'debug.log' file (inside the 'transdeluxe' directory) and make it writable.");
...
function gltr_debug_ua($msg) {
  if (DEBUG_UA) {
    ...
    $fh = fopen($myFile, 'a') or die("Can't open debug file. Please manually create the 'ua.log' file (inside the 'transdeluxe' directory) and make it writable.");
    $ua_simple = preg_replace("/(.*)\s\(.*/","\\1",$_SERVER['HTTP_USER_AGENT']);
    //fwrite($fh, $today . " [from: ".$_SERVER['REMOTE_ADDR']."|$ua_simple] - [mem:" . memory_get_usage() . "] " . $msg . "\n");
    fwrite($fh, $today . " [from: ".$_SERVER['REMOTE_ADDR']."|$ua_simple] - " . $msg . "\n");
    fclose($fh);
...
function widget_global_translator_init() {
  ...
  register_sidebar_widget('TransDeluxe','widget_global_translator');
