...
$location = get_option('siteurl') . '/wp-admin/admin.php?page=transdeluxe/options-translator.php'; // Form Action URI
...
if (isset($_POST['stage'])){
  ...
  if (!is_writeable(dirname(__file__))){
    $message = "Unable to complete TransDeluxe initialization. Please make writable and readable the following directory:
  ...
  if (!is_dir($cachedir)){
    if(!mkdir($cachedir, 0777)){
      $message = "Unable to complete TransDeluxe initialization. Please manually create and make readable and writeable the following directory:
      <ul><li>".WP_CONTENT_DIR."</li></ul>";
    }
  } else if (!is_readable($cachedir) || !is_writable($cachedir) ){
    $message = "Unable to complete TransDeluxe initialization. Please make readable and writeable the following directory:
	  ...
	  foreach($datafiles as $datafile){
	  	if(!is_file($datafile)){
		    if (!gltr_create_file($datafile)){
					$message .= "Unable to complete TransDeluxe initialization. Please create and make readable and writeable the following file:
			    <ul><li>".$datafile."</li></ul><br />";  		
		    }
	  	} else if (!is_readable($datafile) || !is_writeable($datafile)){
				$message .= "Unable to complete TransDeluxe initialization. Please make readable and writeable the following file:
		    <ul><li>".$datafile."</li></ul><br />";  		
	  	}
	  }  
	  */
...
<form name="test"></form>
<div class="wrap">
  <h2><?php _e('TransDeluxe ')?><?php echo($gltr_VERSION);?></h2>
  
	<div style="padding-bottom:10px;margin-top:5px;margin-bottom:10px;border-bottom:1px solid #CCCCCC;">
	by <a target="_blank" href="http://www.transdeluxe.com"><strong>The TransDeluxe Team</strong></a><br/>
	<p>In order to get all of your translated pages indexed quicker, we suggest installing <strong><a target="_blank" href="http://www.arnebrachhold.de/projects/wordpress-plugins/google-xml-sitemaps-generator/">Google XML Sitemaps Generator</a></strong></p>
    ...
    <fieldset class="options">
  	...
  	<fieldset class="options">
  		...
  		<tr><td>
	        	...
	        	TransDeluxe uses a fast, smart, optimized, self-cleaning and built-in caching system in order to drastically reduce the connections to the translation engines.
	    	      ...
	    	      if ($diff_time < 60){
		      		...
		      		*/
						...
						if ($ban_status == 'banned'){
							...
							<li>You haven't added the flags widget on your pages: adding the flags bar is mandatory in order to make TransDeluxe able to work correctly</li></font>");
    ...
    <fieldset class="options">
  	...
  	<fieldset class="options">
        ...
        <label><?php _e('Block "bad" web spiders and crawlers') ?>
	        	...
	        	<br />	        	<br />
	        	By enabling this option, TransDeluxe will block the access to the translated pages to a lot of "bad" web spiders.
  	...
  	<fieldset class="options">
        ...
        <label>
	        	...
	        	<br /><br />
	        	By enabling this option, TransDeluxe will automatically provide the translated urls to the "<strong>Google XML Sitemaps Generator for WordPress</strong>" plugin.<br />
  	...
  	<fieldset class="options">
        ...
        <label><?php _e('Enable debug') ?>
	        	...
	        	By enabling this option, TransDeluxe will trace all its activities on the <strong>"debug.log"</strong> file, which will be saved in the following directory:<br/>
